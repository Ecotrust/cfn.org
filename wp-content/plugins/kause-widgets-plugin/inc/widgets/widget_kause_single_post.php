...
/**************************************
WIDGET: kause_single_post
***************************************/

	add_action('widgets_init', 'register_widget_kause_single_post' );
	function register_widget_kause_single_post () {
		register_widget('kause_single_post');	
	}

	class kause_single_post extends WP_Widget {
		...
		***************************************/
		function __construct () {
				...
				$widget_ops = array(
					'classname' => 'kause_single_post', 								
					'description' => __('Displays a single post', "loc_kause_widgets_plugin")	 				
				);
				$control_ops = array(
					'width' => 300, 
					'height' => 350, 
					'id_base' => 'kause_single_post' 														
				);

				$this->WP_Widget('kause_single_post', __('Kause: Single Post', "loc_kause_widgets_plugin"), $widget_ops, $control_ops );	
		}
		...
		***************************************/
		function form($instance) {
			...
			?>
				...
				<p>
					<label for="<?php echo $this->get_field_id('orderby'); ?> "><?php _e("Order posts by", "loc_kause_widgets_plugin"); ?>:</label><br>
					<select name="<?php echo $this->get_field_name('orderby'); ?>"> 
		     			<option value="post_date" <?php if (isset($orderby)) {if ($orderby == "date") echo "selected='selected'";} ?>><?php _e("Date", "loc_kause_widgets_plugin"); ?></option> 
		     			<option value="title" <?php if (isset($orderby)) {if ($orderby == "title") echo "selected='selected'";} ?>><?php _e("Title", "loc_kause_widgets_plugin"); ?></option> 
					</select> 

					<select name="<?php echo $this->get_field_name('order'); ?>"> 
		     			<option value="DESC" <?php if (isset($order)) {if ($order == "DESC") echo "selected='selected'";} ?>><?php _e("Descending", "loc_kause_widgets_plugin"); ?></option> 
		     			<option value="ASC" <?php if (isset($order)) {if ($order == "ASC") echo "selected='selected'";} ?>><?php _e("Ascending", "loc_kause_widgets_plugin"); ?></option> 
					</select> 
				</p>

				<p><i>(Takes effect on save)</i></p>
				<hr>
				...
				<p>	
					<label for="<?php echo $this->get_field_id('post_ID'); ?> "><?php _e("Display post", "loc_kause_widgets_plugin"); ?>:</label><br>
				...
				<p>
					...
					<label for="<?php echo $this->get_field_id( 'use_short_excerpt' ); ?>"><?php _e("Use short excerpt?", "loc_kause_widgets_plugin"); ?></label>
				</p>
		...
		***************************************/
		function widget($args, $instance) {
            ...
            <a href="<?php echo get_permalink($this_post->ID); ?>" class="more"><?php _e("more", "loc_kause_widgets_plugin"); ?></a>
