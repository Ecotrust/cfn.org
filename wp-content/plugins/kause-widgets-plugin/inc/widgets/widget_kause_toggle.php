...
***************************************/
	...
	add_action('widgets_init', 'register_widget_kause_toggle' );
	function register_widget_kause_toggle () {
		register_widget('kause_toggle');	
	}

	class kause_toggle extends WP_Widget {
		...
		***************************************/
		function __construct () {
				...
				$widget_ops = array(
					'classname' => 'kause_toggle', 								
					'description' => __('Displays a toggle', "loc_kause_widgets_plugin")	 				
				);
				$control_ops = array(
					'width' => 300, 
					'height' => 350, 
					'id_base' => 'kause_toggle' 														
				);

				$this->WP_Widget('kause_toggle', __('Kause: Toggle', "loc_kause_widgets_plugin"), $widget_ops, $control_ops );	
		}
		...
		***************************************/
		function form($instance) {
			...
			//defaults
			$defaults = array( 
				'widget_title' 	=> __('Toggle', "loc_kause_widgets_plugin"),
				'toggle'		=> array(
					0				=> array(
			...
			?>
				...
				<?php _e("toggle sections", "loc_kause_widgets_plugin"); ?>:
		...
		***************************************/
		function widget($args, $instance) {
			...
			// DEFAULTS
			if (empty($instance)) {
				$widget_title 		= __('Toggle', "loc_trades_widgets_plugin");
				$toggle				= array(
					0					=> array(
						'title'				=> "Toggle Trigger",
						'content'			=> "",
    			...
    			<ul class="toggle">
    				...
    				<?php
    					...
    					for ($i = 0; $i < count($toggle); $i++) {  
				            ...
				            // WPML
				            if (function_exists('icl_translate')) { $toggle[$i]['title'] = icl_translate('loc_kause_widgets_plugin', "$widget_id-widget[$i][title]", $toggle[$i]['title']); }
				            if (function_exists('icl_translate')) { $toggle[$i]['content'] = icl_translate('loc_kause_widgets_plugin', "$widget_id-widget[$i][content]", $toggle[$i]['content']); }
